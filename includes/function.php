...
function processOrder($srow) {
	...
	if($tool['is_curl']==1){
		$url = $tool['curl'];
		$url = str_replace('[input]', $input[0], $url);
		$url = str_replace('[input2]', $input[1], $url);
		$url = str_replace('[input3]', $input[2], $url);
		$url = str_replace('[input4]', $input[3], $url);
		$url = str_replace('[input5]', $input[4], $url);
		$url = str_replace('[name]', $tool['name'], $url);
		$url = str_replace('[money]', $tool['money'], $url);
		$url = str_replace('[time]', time(), $url);
		get_curl($url);
	}
	$row=$DB->get_row("select * from shua_orders where tid='{$srow['tid']}' and input='{$input[0]}' order by id desc limit 1");
	if($row['input'] && $row['status']==0){
		$DB->query("update `shua_orders` set `value`=`value`+1 where `id`='{$row['id']}'");
	}else{
		$DB->query("insert into `shua_orders` (`tid`,`input`,`input2`,`input3`,`input4`,`input5`,`value`,`addtime`,`status`) values ('".$srow['tid']."','".$input[0]."','".$input[1]."','".$input[2]."','".$input[3]."','".$input[4]."','1','".$date."','0')");
	}
}
?>